...
void CGWSvrParser::ProcessMessage(YTSvrLib::IUDPBASE* pSocket, const char *pBuf, int nLen)
{
	if (nLen > 1)//正常数据包
	{
...
void CGWSvrParser::ProcessEvent(YTSvrLib::EM_MESSAGE_TYPE emType, YTSvrLib::IUDPBASE* pConn) {
...
YTSvrLib::IUDPCONNECTOR* CGWSvrParser::AllocateConnector(std::string dstIP)
{
...
void CGWSvrParser::ReleaseConnector(YTSvrLib::IUDPCONNECTOR* pConn)
{
	CGWSvrSocket* pGWSvrSocket = dynamic_cast<CGWSvrSocket*>(pConn);
	if (pGWSvrSocket)
	{
