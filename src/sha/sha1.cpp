...
namespace YTSvrLib
{
	...
	*/
	...
	void SHA1::finalize(uint8_t out[20])
	{
		...
		uint32_t out_digest[5] = { 0 };
		for (size_t i = 0; i < _countof(digest); ++i)
		{
			out_digest[i] = htonl(digest[i]);
		}

		for (size_t i = 0; i < _countof(digest); ++i)
		{
			memcpy(out + sizeof(uint32_t)*i, &out_digest[i], sizeof(uint32_t));
		}
	...
	{
		YTSvrLib::SHA1 checksum;
		checksum.update(s);

		return checksum.finalize();
	}

	void SHA1HASH(const std::string &s, uint8_t out[20])
	{
		YTSvrLib::SHA1 checksum;
		checksum.update(s);

		checksum.finalize(out);
	}
}