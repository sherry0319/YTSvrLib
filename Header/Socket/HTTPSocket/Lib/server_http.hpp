...
namespace SimpleWeb {
        ...
        asio::async_write(*self->session->connection->socket, *send_queue.begin()->first, [self](const error_code &ec, std::size_t /*bytes_transferred*/) {
            ...
            return;
          {
            ...
            if(!ec) {
			  ...
			  if (self->close_connection_after_response && self->send_queue.size() == 0)
			  {
  ...
  public:
    ...
    void stop() noexcept {
      ...
      if(acceptor) {
        ...
        {
          LockGuard lock2(connections->mutex);
  ...
  protected:
    ...
    std::shared_ptr<Connection> create_connection(Args &&... args) noexcept {
      ...
      auto connection = std::shared_ptr<Connection>(new Connection(handler_runner, std::forward<Args>(args)...), [c](Connection *connection) {
        {
          LockGuard lock(c->mutex);
          auto it = c->set.find(connection);
          if(it != c->set.end())
            c->set.erase(it);
        }
        delete connection;
      ...
      if(on_upgrade) {
        ...
        if(it != session->request->header.end()) {
          // remove connection from connections
          {
            ...
            auto it2 = connections->set.find(session->connection.get());
            if(it2 != connections->set.end())
              connections->set.erase(it2);
          }
